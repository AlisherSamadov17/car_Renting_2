...
func (h Handler) GetAvaibleCars(c *gin.Context) {
	...
	request.Page = page
	request.Limit = limit
	cars, err := h.Store.Car().GetAvaibleCars(context.Background(),request)
	if err != nil {
		handleResponse(c, "error while gettign cars", http.StatusBadRequest, err.Error())
		return
	}
...
func (h Handler) GetByIDCar(c *gin.Context) {
	id := c.Param("id")
	fmt.Println("id:", id)
	car, err := h.Store.Car().GetByID(context.Background(),id)
	if err != nil {
...
func (h Handler) DeleteCar(c *gin.Context) {
	...
	err = h.Store.Car().Delete(context.Background(),id)
	if err != nil {
